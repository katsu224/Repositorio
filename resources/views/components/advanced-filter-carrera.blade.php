                ...
                <li>
                    <div class="flex p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                        <div class="flex items-center h-5">
                            @php
                                $sortParams = array_merge(['sort' => $value, 'items_per_page' => request('items_per_page', $defaultItemsPerPage)], $params ?? []);
                            @endphp
                            <a href="{{ route($route, $sortParams) }}" class="flex items-center">
            ...
            @foreach ([5, 10, 20, 40, 60, 80, 100] as $number)
                <li>
                    <div class="flex p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                        <div class="flex items-center h-5">
                            @php
                                $pageParams = array_merge(['sort' => request('sort', 'asc'), 'items_per_page' => $number], $params ?? []);
                            @endphp
                            <a href="{{ route($route, $pageParams) }}" class="flex items-center">
